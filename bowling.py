...
def score_tenth_frame(frame):
    # Case: first strike
    if frame[0] == 10:
        # Case: second strike
        if frame[1] == 10:
            score = frame[2] + (frame[1] + frame[2]) + \
                (frame[0] + frame[1] + frame[2])
            return score
        # Case: second not strike
        else:
            score = frame[2] + frame[1] + (frame[2] + frame[1] + frame[0])
            return score
    # Case: spare
    elif frame[0] + frame[1] == 0:
        score = frame[2] + (frame[2] + frame[1] + frame)
        return score
    # Case: all else
    else:
        score = frame[1] + frame[0]
        return score


def score_game(frame_set):
    game_score = 0
    for frame in frame_set:
        if frame_set.index(frame) == 9:
            if frame[0] == 10:
